...
import { getDetail, getTvStream } from "../../api";
...
export default function TvDetail({id, kind, category}:IDetailProps) {

  const tvIdMatch = useRouteMatch<{tvId:string}>([
    "/tv/airingToday/:tvId",
    "/tv/pupular-TV/:tvId",
    "/tv/topRated-TV/:tvId"
  ])
  ...
  const {data: TvStreamData } = useQuery<IMovieVideo>(
    ["TvStream","Tv"], () => getTvStream(tvIdMatch?.params.tvId)) 
  console.log(TvStreamData)
  const [isSound] = useRecoilState(isSoundAtom);
